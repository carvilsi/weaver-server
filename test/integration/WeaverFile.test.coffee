...
describe 'WeaverFile rest-API test', ->
  ...
  it 'should retrieve a file by fileName', ->
    weaverServer
    .get('/file/download?payload={"fileName":"' + file + '","target":"area51","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
    .expect(200)
    .then((res) ->
  ...
  it 'should fails retrieving a file, because the file does not exits on server', ->
    weaverServer
    .get('/file/download?payload={"fileName":"foo.bar","target":"area51","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
    .expect(503)
    .then((res) ->
  ...
  it 'should fails retrieving a file, because the project does not exits on server', ->
    weaverServer
    .get('/file/download?payload={"fileName":"' + file + '","target":"area56","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
    .expect(503)
    .then((res, err) ->
      error = JSON.parse(res.text)
      if res.error
  ...
  it 'should retrieve a file by ID', ->
    fileId = "#{file}".split('-')[0]
    weaverServer
    .get('/file/downloadByID?payload={"id":"' + fileId + '","target":"area51","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
    .expect(200)
    .then((res) ->
      json = JSON.stringify(res.body)
      dataJson = JSON.parse(json)
  ...
  it 'should fails retrieving a file by ID, because there is no file matching this ID', ->
    weaverServer
    .get('/file/downloadByID?payload={"id":"555","target":"area51","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
    .expect(503)
    .then((res) ->
      error = JSON.parse(res.text)
      if res.error
  ...
  it 'should deletes a file by name', ->
    ...
    .post('/file/delete')
    .type('json')
    .send('{"fileName":"' + file + '","target":"area51","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
    .expect(200)
  ...
  it 'should deletes a file by id', ->
    ...
    .then((res) ->
      ...
      .type('json')
      .send('{"id":"' + fileId + '","target":"area51","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
      .expect(200)
    )
  ...
  it 'should fails trying to delete a file because the project does not exists', ->
    ...
    .post('/file/delete')
    .type('json')
    .send('{"fileName":"' + file + '","target":"area56","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
    .expect(503)
    .then((res) ->
      error = JSON.parse(res.text)
      if res.error
  ...
  it 'should fails trying to delete a file by ID because the project does not exists', ->
    ...
    .post('/file/deleteByID')
    .type('json')
    .send('{"id":"' + file + '","target":"area56","accessToken":"eyJhbGciOiJSUzI1NiJ9.eyIkaW50X3Blcm1©"}')
    .expect(503)
    .then((res) ->
      error = JSON.parse(res.text)
      if res.error
