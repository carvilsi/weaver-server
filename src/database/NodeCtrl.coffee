...
bus.private('node.clone').retrieve('database', 'user', 'project').require('sourceId', 'targetId', 'relationsToTraverse', 'sourceGraph', 'targetGraph').on((req, database, user, project, sourceId, targetId, relationsToTraverse, sourceGraph, targetGraph) ->
  AclService.assertACLWritePermission(user, project.acl)
